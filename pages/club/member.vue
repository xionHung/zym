<template>
		<PageComp title="成员" :isApplique="false" navStyle="background: #fff" >
			<mescroll-uni :fixed="false" ref="mescrollRef" @init="mescrollInit" @down="downCallback" @up="upCallback">
				<view class="content">
					...
					<view class="item flex" v-for="(item, index) in listData" :key="index">
						<md-avatar :width="80" :sex="item.sex" :src="item.headpic" @click="jump(item.uid)" />
						<view class="conter">
							<view class="mb-12">
							...
							<view class="flex">
								...
								<view class="label label-box flex-center" v-for="(j, k) in item.label" :key="k">{{ j }}</view>
							</view>
						</view>
					</view>
				</view>
...
<script>
	...
	export default {
      ...
      jump(user_id) {
        uni.navigateTo({ url: '/pages/mine/person/person?user_id=' + user_id });
      }
		}
	}
</script>
