<template>
	<view class="simple-date-picker" v-if="done">
	...
	export default {
		...
		methods: {
			...
			initYears() {
				...
				if (this.value[0] > years[years.length - 1]) {
					this.$set(value, 0, years[years.length - 1]);
				}
				this.years = years;
			},
			...
			initDays() {
				...
				if (this.value[2] > days[days.length - 1]) {
					this.$set(value, 2, days[days.length - 1]);
				}
				this.days = days;
			},
			...
			show() {
				this.$nextTick(() => {
					console.log(new Date(this.defaultValue).toLocaleString(), 233333)
					this.init();
					this.open = true;
				})
			},
			...
			_onSubmit() {
				...
				let timeStr = this.format.replace(/(\w+)/g, '{$1}').replace(/{([yMdhms])+}/g, (result, key) => {
					console.log(result, key);
					let val = formatObj[key]
					return val.toString().padStart(result.length - 2, '0')
				})
				const result = {
					time: date.getTime(),
					timeStr,
